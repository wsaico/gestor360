            ...
            <AnimatePresence>
                {(showFilters || window.innerWidth >= 640) && (
                    <motion.div
                        ...
                        className="card p-4 space-y-4 md:space-y-0 md:flex md:flex-wrap md:items-end md:gap-4 overflow-hidden"
                    >
                        ...
                        orders.map((order) => (
                            ...
                            >
                                ...
                                <div className="grid grid-cols-2 gap-2 py-3 border-t border-b border-gray-50 dark:border-gray-700/50">
                                    <div>
                                        <p className="text-[10px] text-gray-400 uppercase font-bold tracking-wider">Fecha & Tipo</p>
                                        <p className="text-sm font-semibold text-gray-700 dark:text-gray-300">
                                            {formatDate(order.menu_date)} - <span className="text-primary-600">{MEAL_TYPE_LABELS[order.meal_type] || order.meal_type}</span>
                                        </p>
                                    </div>
                                    <div className="text-right">
                                        <p className="text-[10px] text-gray-400 uppercase font-bold tracking-wider">Opción</p>
                                ...
                                <div className="flex gap-3">
                                    ...
                                    {(order.status === 'PENDING' || order.status === 'CONFIRMED') && (
                                        <button
                                            onClick={() => handleStatusChange(order.id, 'CANCELLED')}
                                            className={`py-3 px-6 rounded-xl font-bold transition-all active:scale-95 flex items-center justify-center gap-2 ${order.status === 'PENDING' && isManager
                                                ? 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300'
                                                : 'flex-1 bg-red-50 dark:bg-red-900/20 text-red-600 border border-red-100 dark:border-red-900/30'
                                                }`}
                                        >
                                            <XCircle className="w-5 h-5" />
                                            <span>Cancelar</span>
                                    ...
                                    {/* Delete option in mobile ellipsis or simplified */}
                                    {hasRole(ROLES.ADMIN) && (
                                        <button
                                            onClick={() => handleDelete(order.id)}
                                            className="p-3 bg-gray-50 dark:bg-gray-700 text-gray-400 rounded-xl hover:text-red-500"
                                        >
                                            <Trash2 size={20} />
                                        </button>
                                    )}
