-- MIGRATION: 20241223_add_provider_validation.sql
-- OBJECTIVE: Add provider validation columns to transport_schedules and update RPC
-- DATE: 2025-12-22

BEGIN;

-- 1. Add Columns to transport_schedules
ALTER TABLE public.transport_schedules
ADD COLUMN IF NOT EXISTS is_provider_validated BOOLEAN DEFAULT FALSE,
...
BEGIN
  ...
  -- 2. Build Query
  -- Aggregate result with all relations (Route + Organization, Provider, Station, Execution, DRIVER, VEHICLE)
  SELECT jsonb_agg(
    to_jsonb(s) || jsonb_build_object(
       -- NEW: Explicit validation status return (though select * covers it, ensuring it exists)
       'is_provider_validated', s.is_provider_validated,
